use pylight::{PythonParser, Symbol, SymbolKind};
use std::path::Path;
...
fn test_extract_simple_symbols() {
    ...
    let symbols = parser.parse_file(path, content).expect("Failed to parse file");
    ...
    // Verify symbol kinds
    let simple_func = symbols.iter().find(|s| s.name == "simple_function").unwrap();
    ...
    let method = symbols.iter().find(|s| s.name == "method" && s.container_name.is_some()).unwrap();
...
fn test_extract_nested_symbols() {
    ...
    assert_eq!(deeply_nested.container_name.as_deref(), Some("outer_function.inner_function"));
    ...
    assert_eq!(inner_method.container_name.as_deref(), Some("OuterClass.InnerClass"));
}
...
fn test_decorated_symbols() {
    ...
    for name in decorated_names {
        assert!(symbols.iter().any(|s| s.name == name), "Missing decorated function: {}", name);
    }
}